...
test('creates an asset and edits identity, assignment, purchase, status, and custom fields in the UI', async ({ page, request }) => {
  ...
  await page.getByLabel(/Base Status/).selectOption('IN_USE')
  await page.getByLabel(/Custom Status/).selectOption(status.id)
  ...
  await page.getByLabel(/Current Location/).fill('Yard B')
