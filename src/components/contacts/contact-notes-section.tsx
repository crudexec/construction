...
async function fetchContactComments(contactId: string): Promise<ContactComment[]> {
...
async function createComment(contactId: string, content: string) {
  const token = document.cookie
    .split('; ')
    .find(row => row.startsWith('auth-token='))
    ?.split('=')[1]
...
async function updateComment(commentId: string, content: string) {
  ...
  const response = await fetch(`/api/contacts/comments/${commentId}`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`
...
export function ContactNotesSection({ contactId }: ContactNotesSectionProps) {
  ...
  const { data: comments = [], isLoading } = useQuery({
    ...
    queryFn: () => fetchContactComments(contactId),
    enabled: !!contactId
  })

  const createMutation = useMutation({
    mutationFn: (content: string) => createComment(contactId, content),
    onSuccess: () => {
